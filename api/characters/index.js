...
async function getCharacters(req, res) {
  try {
    ...
    // Get all characters for the default user, with active character first
  ...
  try {
    ...
    const results = await sql.transaction([
      sql`
        ...
        VALUES (${userId}, ${name.trim()}, ${JSON.stringify(characterData)}, ${setAsActive})
    ...
    const additionalQueries = [
      sql`
        ...
        VALUES (${character.id}, ${userId}, ${JSON.stringify(characterData)}, 'initial')
      `
    ];
...
async function autoSaveCharacter(req, res) {
  try {
    ...
    await sql.transaction([
      ...
      sql`
        INSERT INTO character_saves (character_id, user_id, save_data, save_type)
        VALUES (${characterId}, ${userId}, ${JSON.stringify(characterData)}, 'auto')
      `,
      sql`
...
async function testCharacterSave(req, res) {
  try {
    ...
    const createResult = await sql`
      INSERT INTO characters (user_id, name, character_data, is_active)
      VALUES (${userId}, ${testCharacterData.name}, ${JSON.stringify(testCharacterData)}, false)
    ...
    await sql`
      INSERT INTO character_saves (character_id, user_id, save_data, save_type)
      VALUES (${character.id}, ${userId}, ${JSON.stringify(testCharacterData)}, 'test')
    `;
    ...
    await sql`
      INSERT INTO character_saves (character_id, user_id, save_data, save_type)
      VALUES (${character.id}, ${userId}, ${JSON.stringify(updatedData)}, 'test')
    `;
